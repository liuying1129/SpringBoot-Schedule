...
//定时任务管理
var setupImg = document.querySelector('img[alt="设置"]');
setupImg.onclick = function() {
	window.open("static/taskManage.html","_blank");
}

$(document).ready(function() {
		
	var queryAllJob = function(){
		...
		return queryAllJob;//保证立即执行一次
	};
		
　　　var queryRunningJob = function(){
　　　　　　
		$.ajax({
			//默认值: true。如果需要发送同步请求，请将此选项设置为 false。注意，同步请求将锁住浏览器，用户其它操作必须等待请求完成才可以执行
		async : true,
		...
		return queryRunningJob;//保证立即执行一次
　　　};
	
	//自动刷新界面
	setInterval(queryAllJob(), 5000);
	setInterval(queryRunningJob(), 5000);
